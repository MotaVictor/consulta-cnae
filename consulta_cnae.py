...
# Campo de entrada com botão sutil (form)
...
if submit and codigo:
    ...
    if not resultados.empty:
        ...
        for _, row in resultados.iterrows():
            st.markdown(f"<div style='margin-left:10px;'>• {row['CNT_Classe__c']}</div>", unsafe_allow_html=True)
    else:
...
# Rodapé visível ao final da página
st.markdown("""
